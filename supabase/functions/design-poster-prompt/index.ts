...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fullPrompt = isOccasion ? [
      ...
      `4. MANDATORY BOLD LOGO LOCKUP at the bottom: the EXACT phrase "Powered by Afristall" where the word "Afristall" MUST be rendered as the official Afristall WORDMARK — a custom geometric, EXTRA-BOLD / BLACK-WEIGHT sans-serif (think Inter Black / Helvetica Black / SF Pro Display Heavy), all letters the SAME visual weight as the wordmark, slightly tighter tracking, with "Afristall" visibly heavier and ~10-15% larger than the words "Powered by". The whole lockup minimum ~5% of canvas height, perfectly horizontal, perfectly centered along the bottom safe-zone, in pure high-contrast color (pure white on dark backgrounds, pure black on light backgrounds). It must read as a confident BRAND LOGO / wordmark — not a tiny watermark, never script, never thin, never italic, never blurred, never omitted, never cropped, nothing overlapping it. Same size and same position every time.`,
      ...
      phone
        ...
        : `3. CTA BUTTON — ONE pill-shaped button, ${accent} background, crisp white text reading EXACTLY the 2 words "Order Now". Rounded-full corners. Same size every time. No duplicate buttons.`,
      `4. MANDATORY BOLD LOGO LOCKUP at the bottom: the EXACT phrase "Powered by Afristall" where the word "Afristall" MUST be rendered as the official Afristall WORDMARK — a custom geometric, EXTRA-BOLD / BLACK-WEIGHT sans-serif (Inter Black / Helvetica Black / SF Pro Display Heavy), with "Afristall" visibly heavier and ~10-15% larger than the words "Powered by". Minimum ~5% of canvas height, perfectly horizontal, perfectly centered along the bottom safe-zone, in pure high-contrast color (white on dark, black on light). It must read as a confident BRAND LOGO / wordmark — never script, never thin, never italic, never blurred, never omitted, never cropped, nothing overlapping it. Same size and position every time.`,
      storeName ? `Optional small store name "${storeName}" tucked in a top corner — much smaller than the Powered by Afristall lockup.` : "",
      ...
      try {
        const validatorPrompt = [
          ...
          phone
            ...
            : "",
          `4. LOGO LOCKUP: the EXACT three words "${expectedLogo}" appear along the BOTTOM of the canvas, bold/heavy sans-serif, clearly readable (>= roughly 4% of canvas height), inside its own safe zone with nothing overlapping it, perfectly horizontal, not cropped, not in script.`,
